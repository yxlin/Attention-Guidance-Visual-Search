...
ns <- function(DT, exp) 
{
  length(unique(DT[E == exp, .N, .(I, Q, SS, s)]$s)) 
}


get.slope <- function(RT, ss) 
{
  mod <- lm(RT ~ ss)
  coef(mod)[[2]]
}

sim.effect2015 <- function(df)
{
  v.effect <- summarySEwithin(dplyr::tbl_df(df), 
                              measurevar = 'v', 
                              withinvar   =c('S', 'SS'), 
                              idvar='s', 
                              na.rm = FALSE, conf.interval = .95, .drop=TRUE)
  
  a.effect <- summarySEwithin(dplyr::tbl_df(df), 
                              measurevar = 'a', 
                              withinvar   =c('S', 'SS'), 
                              idvar='s', 
                              na.rm = FALSE, conf.interval = .95, .drop=TRUE)
  t.effect <- summarySEwithin(dplyr::tbl_df(df), 
                              measurevar = 't0', 
                              withinvar   =c('S', 'SS'), 
                              idvar='s', 
                              na.rm = FALSE, conf.interval = .95, .drop=TRUE)
  
  prt <- cbind( v.effect[, 1:3], round(v.effect[, 4:5], 3),
                round(a.effect[, 4:5], 3),
                round(t.effect[, 4:5], 3))
  prt
