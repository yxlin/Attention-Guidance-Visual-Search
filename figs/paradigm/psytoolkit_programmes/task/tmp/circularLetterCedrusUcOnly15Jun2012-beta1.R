...
diameter = 100    # define diameter. You may need to change it, if used in eye tracking and ERP studies.


#----------------------------------------------------------------------
# Functions defined in Stoet's orginal visual search programme
#----------------------------------------------------------------------
# The maxsetsize should set at 20 because the number of presented images were manually coded in psy file
minsetsize = 3
maxsetsize = 20

# In line 98-114, we've used sample to randomise the position.
printstimuli=function( stimuli, positions ) {
 positiontablex = positiontable * diameter
 l = length(stimuli)
 for( i in 1:l )  {
     cat( stimuli[i] , positiontablex[ positions[i] , 1 ] , positiontablex[ positions[i] , 2 ] , " ", file="TrialSeqNonRanStiTab.txt", append=T)
   }
}
...
printemptystimuli=function(n) {
 if ( n > 0 ) {
    for ( i in 1:n )
      cat( " empty 250 250 ", file="TrialSeqNonRanStiTab.txt", append=T)
   }
}
...
# Experimental Design 
# ----------------------------
# One block contains a trail from each combination of the condition.
# The first block serves as the practice.  Valid trail number is 480, which would take about 30 minutes to complete.
# The test sequence is (1) 1 block of practice, (2) 10 blocks of baseline condition 

# --------------------------------------------------------------------------------------
# Each letter is presented in a cicular position. We excluded two position 
# (theta = 0 and pi), to avoid the ambiguious judgement  
# 16 letters were allocated to one of the 18 possible positions, which are coded in a table 


#--------------------------------------------------------------
#--- Design information---
#--------------------------------------------------------------
# 1. Set iLength to 11. We produce practice and formal test altogether. First block serves as prc
# 2. isiSet decides how the interval stimulus interval 
# 3. setsizeSet decides setsize
# 4. targetPosSet defines the target position (left/right)
# 5. cueSet defines uppercase cue (1) or lowercase cue (2)
# 6. congrSet defines the congruence between the cue and search set.  If congruent (1), the letters in cue and in search set are the same form,
# whereas, they are in the opposite form, if incongruent (2)
# 7. competingSet defines if the programme presents the competing letter in the search set.  In congruent condition, competing (1) presents an 
# opposite form, but same meaning letter (A-a/a-A).  In incongruent condition when an observer should search the same meaning, but opposite form
# of target, competing (1) presents a identical letter as the competing distractor (A-A/a-a).  In both congruent and incongruent conditions, the non-competing presents only the target letter (same form/opposite form) and other non-identical letters as distractors.     
# 8. bRSize defines the trial number in each block.  This has to be defines explicitlly because we latter used it to randomise condition within 
# one block.
# 9. previewSwitch has not yet used.  It is possibly for the third (future) study.
...
for ( previewOrNot in previewSwitch ) 
{
 for ( i in 1:iLength )   
  {  # i = 1
  previousSetpoint <- countervalue          # countervalue is used to randomise trial within a block
  for (isi in isiSet) 
    {  # isi = 50 
    for (setsize in setsizeSet)             
      ...
      for (targetPos in targetPosSet)       # 5-left; 6-right
        ...
        #--------------------------------
          ...
          cat("  ",counter(), targetPos, setsize, isi, cueImg, cue, congr, competing, " ", file="TrialSeqNonRanStiTab.txt", append=T)
          ...
          cat("\n", file="TrialSeqNonRanStiTab.txt", append=T)
          #--------------------------------------------------------
    ...
    tabTmp <- read.table("TrialSeqNonRanStiTab.txt", colClasses = rep('character', 5), header=F)
    ...
    write.table(blockRandomise, file="blockRandomised.txt", col.names=F, row.names=F, quote=F, append=T)
